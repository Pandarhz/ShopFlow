...
class TestOrders:
    ...
    def test_coupon_inexistant_retourne_404(self, client):
        ...
        p = self._setup_panier(client, user_id=207)
        response = client.post('/orders/', json={
            'user_id': 207,
            'coupon_code': 'FAKECODE'
        })
        assert response.status_code == 404
        assert 'Coupon non trouvé' in response.json().get('detail', '')

    def test_get_commande_par_id(self, client):
        """Créer une commande puis la récupérer par son id."""
        p = self._setup_panier(client, user_id=208)
        order = client.post('/orders/', json={'user_id': 208}).json()
        response = client.get(f'/orders/{order["id"]}')
        assert response.status_code == 200
        data = response.json()
        assert data['id'] == order['id']
        assert data['total_ttc'] == order['total_ttc']
