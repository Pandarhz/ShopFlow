...
class TestUpdateDeleteProduct:
    ...
    def test_filtre_prix_min_max(self, client):
        ...
        client.post('/products/', json={'name': 'Bon prix', 'price': 100.0, 'stock': 5})
        client.post('/products/', json={'name': 'Trop cher', 'price': 250.0, 'stock': 2})
        client.post('/products/', json={'name': 'Trop bas', 'price': 25.0, 'stock': 10})

        response = client.get('/products/?min_price=50&max_price=200')
        assert response.status_code == 200
        produits = response.json()
        assert any(p['name'] == 'Bon prix' for p in produits)
        assert all(50.0 <= p['price'] <= 200.0 for p in produits)
        assert all(p['name'] != 'Trop cher' for p in produits)
        assert all(p['name'] != 'Trop bas' for p in produits)
