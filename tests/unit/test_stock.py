...
class TestReserverStock:
    def test_reservation_reussie(self, product_sample, db_session, monkeypatch):
        """
        ...
        mock_redis = Mock()
        monkeypatch.setattr(REDIS_MOCK_PATH, mock_redis)
    ...
    def test_reservation_verifie_cle_redis(self, product_sample, db_session, monkeypatch):
        """
    ...
    def test_stock_insuffisant_leve_exception(self, product_sample, db_session, monkeypatch):
        """
        ...
        """
        monkeypatch.setattr(REDIS_MOCK_PATH, Mock())
        with pytest.raises(ValueError, match='insuffisant'):
            reserver_stock(product_sample, 999, db_session)
    def test_stock_insuffisant_ne_modifie_pas_bdd(self, product_sample, db_session, monkeypatch):
        """
    ...
    def test_redis_non_appele_si_exception(self, product_sample, db_session, monkeypatch):
        """
    ...
    def test_liberation_stock(self, product_sample, db_session, monkeypatch):
        """
    ...
    def test_liberer_stock_quantite_invalide_leve_exception(self, product_sample, db_session):
        with pytest.raises(ValueError, match='invalide'):
            liberer_stock(product_sample, 0, db_session)
        with pytest.raises(ValueError, match='invalide'):
            liberer_stock(product_sample, -1, db_session)
